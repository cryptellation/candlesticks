...
func (wf *workflows) ListCandlesticksWorkflow(
	...
	ranges, err := timeseries.MergeTimeRanges(missingRanges, uncompleteRanges)
	if err != nil {
		return api.ListCandlesticksWorkflowResults{}, err
	}
	...
	logger.Debug("Candlesticks are missing from DB",
		"missing time ranges", timeseries.TimeRangesToString(ranges))
...
func validateCandlesticksParams(
	...
	if params.End == nil || params.End.After(workflow.Now(ctx)) {
		...
		params.End = timeToReference(workflow.Now(ctx))
	}
	...
	if params.Start == nil || params.Start.After(*params.End) {
		...
		params.Start = timeToReference(
			params.End.Add(-params.Period.Duration() * MinimalRetrievedMissingCandlesticks))
	}

	// Round down params start and end
	params.Start = timeToReference(params.Period.RoundTime(*params.Start))
	params.End = timeToReference(params.Period.RoundTime(*params.End))

	return params, nil
}

func timeToReference(t time.Time) *time.Time {
	if t.IsZero() {
		return nil
	}
	return &t
...
// TODO(#17): Refactor this function to make it shorter.
//
